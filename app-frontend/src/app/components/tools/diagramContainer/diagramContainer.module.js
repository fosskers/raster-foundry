...
import { nodesFromTool, getNodeDefinition } from '_redux/node-utils';
...
class DiagramContainerController {
    ...
    $onInit() {
        ...
        const toolDefWatch = this.$scope.$watch('$ctrl.tool', (tool) => {
            if (tool && !this.shapes) {
                this.initDiagram();
            } else {
                toolDefWatch();
    ...
    initDiagram() {
        let definition = this.tool.executionParameters ?
            this.tool.executionParameters :
            this.tool;
        ...
        let toolNodes = nodesFromTool(definition);
        this.initNodes(toolNodes);
